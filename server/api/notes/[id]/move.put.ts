...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.log('[API] Updating note folder_id', {
      noteId,
      oldFolderId,
      newFolderId: body.newFolderId
    });
    ...
    const updateResult = await executeQuery(
      'UPDATE notes SET folder_id = ?, updated_at = NOW() WHERE id = ?',
    ...
    const newFolderNotes = await executeQuery<any[]>(
      `SELECT id FROM notes WHERE user_id = ? AND ${folderCondition} AND id != ? ORDER BY created_at ASC`,
      [...folderParams, noteId]
    );
