                ...
                <div class="mb-4">
                  <div class="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 p-3 mb-3">
                    ...
                    <ul class="text-xs text-red-700 dark:text-red-400 space-y-1 list-disc list-inside">
                      <li>All <strong>notes</strong> in this section will be permanently deleted</li>
                      <li v-if="noteCount > 0"><strong>{{ noteCount }} note{{ noteCount === 1 ? '' : 's' }}</strong> will be lost</li>
                      <li>This action cannot be undone</li>
                    </ul>
                  </div>

                  <p class="text-xs text-gray-700 dark:text-gray-300">
                    Are you absolutely sure you want to delete <strong class="text-gray-900 dark:text-white">"{{ folderName }}"</strong>?
                  </p>
                </div>
...
const folderName = computed(() => {
  ...
  return foldersStore.folders.find(f => f.id === props.folderId)?.name || '';
});

const noteCount = computed(() => {
  if (!props.folderId) return 0;
  return notesStore.notes.filter(n => n.section_id === props.folderId).length;
});
</script>
